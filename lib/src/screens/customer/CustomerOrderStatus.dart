...
class _CustomerOrderStatusState extends State<CustomerOrderStatus> {
  ...
  String _getCurrentStatus(Map<dynamic, dynamic> status) {
    ...
    if (status['Ready For Shipping'] != null) return 'Ready For Shipping';
  ...
  List<Widget> _buildTimeline() {
    final statuses = ['Pending', 'Ready For Shipping', 'Shipping', 'Arrived', 'Completed'];
    final DateFormat formatter = DateFormat('dd MMM yyyy hh:mm a');
    return statuses.map((status) {
      String date = '';
      bool hasTimestamp = _orderData!['completion_status'][status] != null;

      if (hasTimestamp) {
        final DateTime dateTime = DateTime.parse(_orderData!['completion_status'][status]);
        date = formatter.format(dateTime);
      }
      ...
      return TimelineTile(
        ...
        contents: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              if (hasTimestamp)
                Text(
                  _getDetailedStatusDescription({status: _orderData!['completion_status'][status]}),
                  style: const TextStyle(
                    color: Colors.black,
                    fontSize: 11.0,
        ...
        node: TimelineNode(
          ...
          startConnector: statuses.first == status
              ? null
              : hasTimestamp
          ...
          endConnector: statuses.last == status
              ? null
              : hasTimestamp
              ? DecoratedLineConnector(
            decoration: BoxDecoration(
